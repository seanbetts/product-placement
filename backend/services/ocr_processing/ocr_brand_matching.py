...
########################################################
async def detect_brands_and_interpolate(vlogger, cleaned_results: List[Dict], fps: float, video_resolution: Tuple[int, int]) -> Tuple[List[Dict], Dict[str, Dict[int, List[Dict]]]]:
    @vlogger.log_performance
    async def _detect_brands_and_interpolate():
        try:
            ...
            dual_log(vlogger, app_logger, 'info', "OCR Data Processing - Step 3.2: Starting first pass - Detecting brands")
            ...
            for frame in cleaned_results:
                ...
                for detection in frame['cleaned_detections']:
                    # vlogger.logger.debug(f"Examining detection: {detection['text']}")

                    if not detection.get('brand_match'):
                        # vlogger.logger.debug(f"No brand match for: {detection['text']}")
                        continue
                    ...
                    if 'vertices' in box:
                        ...
                        if width_px < min_text_width or height_px < min_text_height:
                            # vlogger.logger.debug(f"Text size ({width_px}x{height_px}) below minimum threshold ({min_text_width}x{min_text_height})")
                            continue
                    else:
                        # vlogger.logger.debug("No bounding box found")
                        continue

                    # vlogger.logger.debug(f"Brand detected: {detection['brand_match']}")
                    brand_data = {
            ...
            dual_log(vlogger, app_logger, 'info', "OCR Data Processing - Step 3.3: Starting second pass - Interpolating brands")
            final_brand_appearances = defaultdict(lambda: defaultdict(list))
            for brand, frame_appearances in brand_appearances.items():
                # vlogger.logger.debug(f"Processing brand: {brand}")
                ...
                if consistent_appearances:
                    ...
                    # vlogger.logger.debug(f"Brand {brand} appears from frame {first_appearance} to {last_appearance}")
                    ...
                    for frame_number in range(first_appearance, last_appearance + 1):
                        if frame_number in frame_appearances:
                            ...
                            # vlogger.logger.debug(f"Frame {frame_number}: {len(frame_appearances[frame_number])} instances of {brand}")
                        else:
                            ...
                            if prev_frame is not None and next_frame is not None:
                                ...
                                interpolated_instances = await interpolate_brand_instances(vlogger, prev_instances, next_instances, prev_frame, next_frame, frame_number, fps)
                                ...
                                if frame_index < len(brand_results):
                                    ...
                                    # vlogger.logger.debug(f"Frame {frame_number}: Interpolated {len(interpolated_instances)} instances of {brand}")

            dual_log(vlogger, app_logger, 'info', "OCR Data Processing - Step 3.4: Removing over interpolated brands")
            # Post-processing to remove over-interpolated sections
            brand_results, final_brand_appearances = await remove_over_interpolated_sections(vlogger, brand_results, final_brand_appearances)

            vlogger.logger.info("Updating brand_results")
            ...
            for frame in brand_results:
                ...
                # vlogger.logger.debug(f"Frame {frame_number}: Final count of {len(frame['detected_brands'])} brands")
            ...
            dual_log(vlogger, app_logger, 'error', f"Error in detect_brands_and_interpolate: {str(e)}", exc_info=True)
            raise

    return await _detect_brands_and_interpolate()
########################################################

## Fuzzy match brands
########################################################
def fuzzy_match_brand(vlogger, text: str, min_score: int = 85) -> Tuple[Optional[str], int]:
    try:
        if text is None:
            vlogger.logger.warning("fuzzy_match_brand received None as input text")
            return None, 0

        cleaned_text = re.sub(r'[^a-zA-Z0-9\s]', '', str(text).lower()).strip()
        if len(cleaned_text) < 3:
            # vlogger.logger.debug(f"Text too short for brand matching: '{text}'")
            return None, 0

        # vlogger.logger.debug(f"Attempting to match brand for text: '{text}', cleaned: '{cleaned_text}'")

        # Check for exact match first
        if cleaned_text in settings.BRAND_DATABASE:
            # vlogger.logger.debug(f"Exact match found for '{cleaned_text}'")
        ...
        for brand, data in settings.BRAND_DATABASE.items():
            ...
            if not isinstance(variations, list):
                vlogger.logger.warning(f"Unexpected 'variations' format for brand '{brand}': {variations}")
                continue

            if cleaned_text in variations:
                # vlogger.logger.debug(f"Variation match found for '{cleaned_text}' in brand '{brand}'")
                return brand, 95

            for variation in variations:
                if cleaned_text in variation or variation in cleaned_text:
                    # vlogger.logger.debug(f"Partial variation match found for '{cleaned_text}' in brand '{brand}'")
                    return brand, 90
        ...
        for brand, data in settings.BRAND_DATABASE.items():
            ...
            if isinstance(variations, list):
                ...
                vlogger.logger.warning(f"Skipping invalid variations for brand '{brand}': {variations}")

        if not all_brand_variations:
            vlogger.logger.warning("No valid brand variations found for fuzzy matching")
            return None, 0
        ...
        if not matching_variations:
            # vlogger.logger.debug(f"No length-appropriate variations found for '{cleaned_text}'")
            return None, 0
        ...
        if score >= min_score:
            ...
            if matched_brand:
                # vlogger.logger.debug(f"Fuzzy match for '{text}': Best match '{best_match}' (brand: {matched_brand}) with score {score}")
                return matched_brand, score
            else:
                # vlogger.logger.debug(f"Matched variation '{best_match}' not found in brand database")
                None
        else:
            # vlogger.logger.debug(f"No fuzzy match found for '{text}' above minimum score {min_score}")
            None

        return None, 0

    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"Error in fuzzy_match_brand for text '{text}': {str(e)}", exc_info=True)
        return None, 0
########################################################

## Interpolate instances where brands appear 2+ times in a single frame
########################################################
async def interpolate_brand_instances(vlogger, prev_instances: List[Dict], next_instances: List[Dict],
                                current_frame: int, next_frame: int, frame_number: int, fps: float) -> List[Dict]:
    try:
        # vlogger.logger.debug(f"Interpolating brand instances for frame {frame_number}")
        matched_pairs = await match_brand_instances(vlogger, prev_instances, next_instances)
        ...
        for prev, next in matched_pairs:
            try:
                if prev and next:
                    ...
                    # vlogger.logger.debug(f"Interpolating between existing instances for frame {frame_number}")
                    interpolated = await interpolate_brand(vlogger, prev, next, current_frame, next_frame, frame_number, fps)
                elif prev:
                    ...
                    dual_log(vlogger, app_logger, 'error', f"No valid instances to interpolate for frame {frame_number}")
                    continue

                if interpolated:
                    interpolated_instances.append(interpolated)
                else:
                    # vlogger.logger.debug(f"Interpolation returned None for frame {frame_number}")
                    continue

            except Exception as e:
                dual_log(vlogger, app_logger, 'error', f"Error interpolating brand instance for frame {frame_number}: {str(e)}", exc_info=True)
                continue

        # vlogger.logger.debug(f"Interpolated {len(interpolated_instances)} instances for frame {frame_number}")
        return interpolated_instances

    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"Error in interpolate_brand_instances for frame {frame_number}: {str(e)}", exc_info=True)
        return []
########################################################

## Detect when brands appear multiple times in a single frame
########################################################
async def match_brand_instances(vlogger, prev_instances: List[Dict], next_instances: List[Dict]) -> List[Tuple[Optional[Dict], Optional[Dict]]]:
    try:
        # vlogger.logger.debug(f"Matching brand instances: {len(prev_instances)} previous, {len(next_instances)} next")
        matched_pairs = []
        ...
        for prev in prev_instances:
            if unmatched_next:
                distances = await asyncio.gather(*[calculate_distance(vlogger, prev['bounding_box'], next['bounding_box']) for next in unmatched_next])
                ...
                # vlogger.logger.debug(f"Matched instance: prev={prev['text']}, next={closest['text']}")
            else:
                matched_pairs.append((prev, None))
                # vlogger.logger.debug(f"Unmatched previous instance: {prev['text']}")

        # Add remaining unmatched next instances
        for next in unmatched_next:
            matched_pairs.append((None, next))
            # vlogger.logger.debug(f"Unmatched next instance: {next['text']}")

        # vlogger.logger.debug(f"Matched {len(matched_pairs)} pairs of instances")
        return matched_pairs

    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"Error in match_brand_instances: {str(e)}", exc_info=True)
        return []
########################################################
...
async def maintain_brand(vlogger, brand: Dict, current_frame: int, next_frame: int, frame_number: int, fps: float) -> Optional[Dict]:
    try:
        ...
        # vlogger.logger.debug(f"Confidence decay factor: {confidence_decay:.4f}")
        ...
        # vlogger.logger.debug(f"Maintained brand '{brand['text']}' for frame {frame_number} with confidence {maintained_brand['confidence']:.4f}")
        return maintained_brand

    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"Error in maintain_brand for frame {frame_number}: {str(e)}", exc_info=True)
        return None
########################################################
...
async def fade_in_brand(vlogger, brand: Dict, current_frame: int, next_frame: int, frame_number: int, fps: float) -> Optional[Dict]:
    try:
        ...
        # vlogger.logger.debug(f"Confidence increase factor: {confidence_increase:.4f}")
        ...
        # vlogger.logger.debug(f"Faded in brand '{brand['text']}' for frame {frame_number} with confidence {faded_in_brand['confidence']:.4f}")
        return faded_in_brand

    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"Error in fade_in_brand for frame {frame_number}: {str(e)}", exc_info=True)
        return None
########################################################
...
async def fade_out_brand(vlogger, brand: Dict, current_frame: int, next_frame: int, frame_number: int, fps: float) -> Optional[Dict]:
    try:
        ...
        # vlogger.logger.debug(f"Faded out brand '{brand['text']}' for frame {frame_number} with confidence {faded_out_brand['confidence']:.4f}")
        return faded_out_brand

    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"Error in fade_out_brand for frame {frame_number}: {str(e)}", exc_info=True)
        return None
########################################################

## Calculate bounding boxes for interpolated brands
########################################################
async def interpolate_brand(vlogger, prev: Dict, next: Dict, current_frame: int, next_frame: int, frame_number: int, fps: float) -> Dict:
    try:
        # vlogger.logger.debug(f"Interpolating brand '{prev['text']}' for frame {frame_number}")

        t = (frame_number - current_frame) / (next_frame - current_frame)
        # vlogger.logger.debug(f"Interpolation factor t: {t:.4f}")
        ...
        # vlogger.logger.debug(f"Interpolated brand '{prev['text']}' for frame {frame_number} with confidence {confidence:.4f}")
        return interpolated_brand

    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"Error in interpolate_brand for frame {frame_number}: {str(e)}", exc_info=True)
...
########################################################
async def calculate_distance(vlogger, box1: Dict, box2: Dict) -> float:
    try:
        # vlogger.logger.debug("Calculating distance between two bounding boxes")
        ...
        # vlogger.logger.debug(f"Calculated distance: {distance:.2f}")
        return distance

    except KeyError as e:
        dual_log(vlogger, app_logger, 'error', f"KeyError in calculate_distance: {str(e)}. Ensure both boxes have 'vertices' with 'x' and 'y' coordinates.", exc_info=True)
        return float('inf')  # Return infinity as a fallback for invalid boxes
    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"Error in calculate_distance: {str(e)}", exc_info=True)
...
########################################################
async def remove_over_interpolated_sections(vlogger, brand_results: List[Dict], final_brand_appearances: Dict[str, Dict[int, List[Dict]]]) -> Tuple[List[Dict], Dict[str, Dict[int, List[Dict]]]]:
    try:
        # vlogger.logger.debug("Starting removal of over-interpolated sections")

        # Get unique brand names
        brand_names = set(brand['text'] for frame in brand_results for brand in frame['detected_brands'])
        # vlogger.logger.debug(f"Found {len(brand_names)} unique brands")

        async def process_brand(brand_name):
            # vlogger.logger.debug(f"Processing brand: {brand_name}")
            interpolated_sequences = await find_interpolated_sequences(vlogger, brand_results, brand_name)
            # vlogger.logger.debug(f"Found {len(interpolated_sequences)} interpolated sequences for {brand_name}")

            for start, end in interpolated_sequences:
                if end - start + 1 > settings.INTERPOLATION_LIMIT:
                    # vlogger.logger.debug(f"Removing over-interpolated section for {brand_name} from frame {start} to {end}")
                        ...
                        for frame_num in range(start, end + 1):
                            if frame_num in final_brand_appearances[brand_name]:
                                ...
                                if not final_brand_appearances[brand_name][frame_num]:
                                    ...
                                    # vlogger.logger.debug(f"Removed empty frame {frame_num} for {brand_name}")
        ...
        vlogger.logger.info(f"Finished processing. {len(final_brand_appearances)} brands remain after removal")

        return brand_results, final_brand_appearances

    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"Error in remove_over_interpolated_sections: {str(e)}", exc_info=True)
...
########################################################
async def find_interpolated_sequences(vlogger, brand_results: List[Dict], brand_name: str) -> List[Tuple[int, int]]:
    try:
        # vlogger.logger.debug(f"Finding interpolated sequences for brand: {brand_name}")
        sequences = []
        start = None
        ...
        for i, chunk in enumerate(chunks):
            ...
            # vlogger.logger.debug(f"Processed chunk {i+1}/{len(chunks)} ({(i+1)*chunk_size}/{total_frames} frames)")

        if start is not None:
            sequences.append((start, brand_results[-1]['frame_number']))
            # vlogger.logger.debug(f"Ended final interpolated sequence: {start} to {brand_results[-1]['frame_number']}")

        # vlogger.logger.debug(f"Found {len(sequences)} interpolated sequences for brand {brand_name}")
        return sequences

    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"Error in find_interpolated_sequences for brand {brand_name}: {str(e)}", exc_info=True)
        return []  # Return an empty list in case of error
########################################################
## Detects if a brand has been interpolated too many times consecutively
########################################################
async def is_over_interpolated(vlogger, brand: Dict, brand_results: List[Dict], current_frame: int) -> bool:
    try:
        # vlogger.logger.debug(f"Checking if brand '{brand['text']}' is over-interpolated at frame {current_frame}")

        if not brand.get('is_interpolated', False):
            vlogger.logger.debug(f"Brand '{brand['text']}' is not interpolated at frame {current_frame}")
            return False

        brand_name = brand['text']
        sequences = await find_interpolated_sequences(vlogger, brand_results, brand_name)

        for start, end in sequences:
            if start <= current_frame <= end:
                sequence_length = end - start + 1
                if sequence_length > settings.INTERPOLATION_LIMIT:
                    vlogger.logger.info(f"Brand '{brand_name}' is over-interpolated at frame {current_frame}. "
                                        f"Sequence length: {sequence_length}, Limit: {settings.INTERPOLATION_LIMIT}")
                    return True
                else:
                    vlogger.logger.debug(f"Brand '{brand_name}' is within interpolation limits at frame {current_frame}. "
                                            f"Sequence length: {sequence_length}, Limit: {settings.INTERPOLATION_LIMIT}")

        # vlogger.logger.debug(f"Brand '{brand_name}' is not over-interpolated at frame {current_frame}")
        return False

    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"Error in is_over_interpolated for brand '{brand['text']}' at frame {current_frame}: {str(e)}", exc_info=True)
