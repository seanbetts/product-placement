    ...
    try:
        ...
        if score >= min_score:
            ...
            if matched_brand:
                ...
                vlogger.logger.warning(f"Matched variation '{best_match}' not found in brand database")
        else:
        ...
        vlogger.logger.error(f"Error in fuzzy_match_brand for text '{text}': {str(e)}", exc_info=True)
        return None, 0
########################################################
...
async def detect_brands_and_interpolate(vlogger, cleaned_results: List[Dict], fps: float, video_resolution: Tuple[int, int]) -> Tuple[List[Dict], Dict[str, Dict[int, List[Dict]]]]:
    ...
    async def _detect_brands_and_interpolate():
        try:
            ...
            vlogger.logger.info("Starting first pass: Detect brands")
            ...
            for frame in cleaned_results:
                ...
                for detection in frame['cleaned_detections']:
                    ...
                    vlogger.logger.info(f"Brand detected: {detection['brand_match']}")
                    brand_data = {
            ...
            for brand, frame_appearances in brand_appearances.items():
                ...
                if consistent_appearances:
                    ...
                    vlogger.logger.info(f"Brand {brand} appears from frame {first_appearance} to {last_appearance}")
            ...
            vlogger.logger.info("Starting post-processing")
            ...
            vlogger.logger.info(f"Final result: {len(final_brand_appearances)} brands detected and interpolated")

            return brand_results, final_brand_appearances

        except Exception as e:
            vlogger.logger.error(f"Error in detect_brands_and_interpolate: {str(e)}", exc_info=True)
            raise
...
async def interpolate_brand_instances(vlogger, prev_instances: List[Dict], next_instances: List[Dict],
    ...
    @vlogger.log_performance
    async def _interpolate_brand_instances():
        try:
            ...
            for prev, next in matched_pairs:
                try:
                    ...
                    vlogger.logger.warning(f"Error interpolating brand instance for frame {frame_number}: {str(e)}", exc_info=True)
                    continue

            vlogger.logger.info(f"Interpolated {len(interpolated_instances)} instances for frame {frame_number}")
            return interpolated_instances

        except Exception as e:
            vlogger.logger.error(f"Error in interpolate_brand_instances for frame {frame_number}: {str(e)}", exc_info=True)
            return []

    return await _interpolate_brand_instances()
########################################################
...
async def match_brand_instances(vlogger, prev_instances: List[Dict], next_instances: List[Dict]) -> List[Tuple[Optional[Dict], Optional[Dict]]]:
    @vlogger.log_performance
    async def _match_brand_instances():
        try:
            vlogger.logger.info(f"Matching brand instances: {len(prev_instances)} previous, {len(next_instances)} next")
            matched_pairs = []
            ...
            vlogger.logger.info(f"Matched {len(matched_pairs)} pairs of instances")
            return matched_pairs

        except Exception as e:
            vlogger.logger.error(f"Error in match_brand_instances: {str(e)}", exc_info=True)
            return []

    return await _match_brand_instances()
########################################################
...
async def maintain_brand(vlogger, brand: Dict, current_frame: int, next_frame: int, frame_number: int, fps: float) -> Optional[Dict]:
    ...
    async def _maintain_brand():
        try:
            ...
            vlogger.logger.info(f"Maintained brand '{brand['text']}' for frame {frame_number} with confidence {maintained_brand['confidence']:.4f}")
...
async def fade_in_brand(vlogger, brand: Dict, current_frame: int, next_frame: int, frame_number: int, fps: float) -> Optional[Dict]:
    ...
    async def _fade_in_brand():
        try:
            ...
            vlogger.logger.info(f"Faded in brand '{brand['text']}' for frame {frame_number} with confidence {faded_in_brand['confidence']:.4f}")
...
async def fade_out_brand(vlogger, brand: Dict, current_frame: int, next_frame: int, frame_number: int, fps: float) -> Optional[Dict]:
    ...
    async def _fade_out_brand():
        try:
            ...
            vlogger.logger.info(f"Faded out brand '{brand['text']}' for frame {frame_number} with confidence {faded_out_brand['confidence']:.4f}")
...
async def interpolate_brand(vlogger, prev: Dict, next: Dict, current_frame: int, next_frame: int, frame_number: int, fps: float) -> Dict:
    ...
    async def _interpolate_brand():
        try:
            ...
            vlogger.logger.info(f"Interpolated brand '{prev['text']}' for frame {frame_number} with confidence {confidence:.4f}")
            return interpolated_brand

        except Exception as e:
            vlogger.logger.error(f"Error in interpolate_brand for frame {frame_number}: {str(e)}", exc_info=True)
            # In case of error, return the previous brand data as a fallback
            return {**prev, "is_interpolated": True, "frame_number": frame_number}

    return await _interpolate_brand()
########################################################
...
async def calculate_distance(vlogger, box1: Dict, box2: Dict) -> float:
    ...
    async def _calculate_distance():
        try:
            ...
            vlogger.logger.error(f"KeyError in calculate_distance: {str(e)}. Ensure both boxes have 'vertices' with 'x' and 'y' coordinates.", exc_info=True)
            return float('inf')  # Return infinity as a fallback for invalid boxes
        except Exception as e:
            vlogger.logger.error(f"Error in calculate_distance: {str(e)}", exc_info=True)
            return float('inf')  # Return infinity as a fallback for any other errors

    return await _calculate_distance()
########################################################
...
async def remove_over_interpolated_sections(vlogger, brand_results: List[Dict], final_brand_appearances: Dict[str, Dict[int, List[Dict]]]) -> Tuple[List[Dict], Dict[str, Dict[int, List[Dict]]]]:
    @vlogger.log_performance
    async def _remove_over_interpolated_sections():
        try:
            vlogger.logger.info("Starting removal of over-interpolated sections")
            ...
            async def process_brand(brand_name):
                ...
                for start, end in interpolated_sequences:
                    if end - start + 1 > settings.INTERPOLATION_LIMIT:
                        vlogger.logger.info(f"Removing over-interpolated section for {brand_name} from frame {start} to {end}")
            ...
            vlogger.logger.error(f"Error in remove_over_interpolated_sections: {str(e)}", exc_info=True)
            return brand_results, final_brand_appearances  # Return original data in case of error

    return await _remove_over_interpolated_sections()
########################################################
...
async def find_interpolated_sequences(vlogger, brand_results: List[Dict], brand_name: str) -> List[Tuple[int, int]]:
    @vlogger.log_performance
    async def _find_interpolated_sequences():
        try:
            vlogger.logger.info(f"Finding interpolated sequences for brand: {brand_name}")
            sequences = []
            start = None
            ...
            vlogger.logger.info(f"Found {len(sequences)} interpolated sequences for brand {brand_name}")
            return sequences

        except Exception as e:
            vlogger.logger.error(f"Error in find_interpolated_sequences for brand {brand_name}: {str(e)}", exc_info=True)
            return []  # Return an empty list in case of error

    return await _find_interpolated_sequences()
########################################################
...
async def is_over_interpolated(vlogger, brand: Dict, brand_results: List[Dict], current_frame: int) -> bool:
    @vlogger.log_performance
    async def _is_over_interpolated():
        try:
            ...
            vlogger.logger.error(f"Error in is_over_interpolated for brand '{brand['text']}' at frame {current_frame}: {str(e)}", exc_info=True)
            return False  # Assume not over-interpolated in case of error

    return await _is_over_interpolated()
########################################################