...
async def filter_brand_results(vlogger, brand_results: List[Dict], brand_appearances: Dict[str, Set[int]], fps: float) -> List[Dict]:
    try:
        # vlogger.logger.debug("Starting to filter brand results")
        ...
        # vlogger.logger.debug(f"Found {len(valid_brands)} valid brands out of {len(brand_appearances)} total brands")
        ...
        # vlogger.logger.debug(f"Filtering complete. Brands reduced from {total_brands_before} to {total_brands_after}")
        return filtered_results

    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"OCR Data Processing: Error in filter_brand_results: {str(e)}", exc_info=True)
...
async def clean_and_consolidate_ocr_data(vlogger, raw_ocr_results: List[Dict], video_dimensions: Tuple[int, int]) -> List[Dict]:
    ...
    async def _clean_and_consolidate_ocr_data():
        try:
            dual_log(vlogger, app_logger, 'info', "OCR Data Processing - Step 1.2: Starting OCR data cleaning and consolidation")
            ...
            dual_log(vlogger, app_logger, 'info', f"OCR Data Processing - Step 1.3: Completed OCR data cleaning and consolidation. Processed {len(cleaned_results)} frames.")
            return cleaned_results

        except Exception as e:
            dual_log(vlogger, app_logger, 'error', f"OCR Data Processing: Error in clean_and_consolidate_ocr_data: {str(e)}", exc_info=True)
...
async def calculate_overlap(vlogger, box1: Dict, box2: Dict) -> float:
    try:
        ...
        dual_log(vlogger, app_logger, 'error', f"OCR Data Processing: KeyError in calculate_overlap: {str(e)}. Ensure both boxes have 'vertices' with correct structure.", exc_info=True)
        return 0.0  # Return no overlap in case of error
    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"OCR Data Processing: Error in calculate_overlap: {str(e)}", exc_info=True)
        return 0.0  # Return no overlap in case of error
########################################################
...
async def should_merge_bounding_boxes(vlogger, box1: Dict, box2: Dict, frame_width: int, frame_height: int) -> bool:
    try:
        ...
        dual_log(vlogger, app_logger, 'error', f"OCR Data Processing: KeyError in should_merge_bounding_boxes: {str(e)}. Ensure both boxes have 'vertices' with correct structure.", exc_info=True)
        return False  # Return False in case of error
    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"OCR Data Processing: Error in should_merge_bounding_boxes: {str(e)}", exc_info=True)
        return False  # Return False in case of error
########################################################
...
async def merge_bounding_boxes(vlogger, box1: Dict, box2: Dict) -> Dict:
    try:
        ...
        # vlogger.logger.debug(f"Merged bounding box: {merged_box}")
        return merged_box

    except KeyError as e:
        dual_log(vlogger, app_logger, 'error', f"OCR Data Processing: KeyError in merge_bounding_boxes: {str(e)}. Ensure both boxes have 'vertices' with correct structure.", exc_info=True)
        return box1  # Return first box in case of error
    except Exception as e:
        dual_log(vlogger, app_logger, 'error', f"OCR Data Processing: Error in merge_bounding_boxes: {str(e)}", exc_info=True)
        return box1  # Return first box in case of error
########################################################